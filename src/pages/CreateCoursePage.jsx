...
import { PlusCircle, XCircle, Save, Image as ImageIcon, Clock, ArrowLeft } from 'lucide-react';
...
const CreateCoursePage = () => {
  ...
  const { addCourse, isLoading: isCourseLoading } = useContext(CourseContext);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!title || !description || !imageUrl || !duration || topics.some(t => !t.name || !t.content)) {
      toast({
        ...
        description: "Please fill in all course details, including image URL, duration, and all topic fields.",
        variant: "destructive",
      });
      return;
    }
    ...
    const newCourseData = {
      ...
      duration,
      topics,
      user_id: user.id, // This will be set in CourseContext if not passed
    };

    const createdCourse = await addCourse(newCourseData);
  ...
  return (
    ...
    >
      ...
      <Card className="bg-secondary border-border shadow-xl">
        <CardHeader>
          <CardTitle className="text-3xl font-bold gradient-text">Create New Course</CardTitle>
          <CardDescription className="text-muted-foreground">
            Fill in the details below to craft your new course. Let your expertise shine!
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 pt-2 border-t border-border/50">
              <h3 className="text-xl font-semibold text-primary/90 pt-2">Course Topics</h3>
